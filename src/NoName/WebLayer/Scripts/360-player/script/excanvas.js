...
if (!document.createElement("canvas").getContext) {
    (function() {
        ...
        function g() { return this.context_ || (this.context_ = new p(this)) }
        ...
        function q() { return[[1, 0, 0], [0, 1, 0], [0, 0, 1]] }
        ...
        function o(i, E, m, j) {
            i.currentPath_.push({ type: "bezierCurveTo", cp1x: E.x, cp1y: E.y, cp2x: m.x, cp2y: m.y, x: j.x, y: j.y });
            i.currentX_ = j.x;
            i.currentY_ = j.y
        }
        ...
        k.quadraticCurveTo = function(H, m, j, i) {
            ...
            var I = { x: this.currentX_ + 2 / 3 * (G.x - this.currentX_), y: this.currentY_ + 2 / 3 * (G.y - this.currentY_) };
            var E = { x: I.x + (F.x - this.currentX_) / 3, y: I.y + (F.y - this.currentY_) / 3 };
            o(this, I, E, F)
        };
        k.arc = function(K, I, J, F, j, m) {
            ...
            this.currentPath_.push({ type: O, x: E.x, y: E.y, radius: J, xStart: H.x, yStart: H.y, xEnd: G.x, yEnd: G.y })
        };
            ...
            if (!ae) {
                ...
                if (typeof this.fillStyle == "object") {
                    ...
                    if (N.type_ == "gradient") {
                        ...
                        var j = V.x - ao.x;
                        var E = V.y - ao.y;
                        ah = { x: (am.x - ao.x) / j, y: (am.y - ao.y) / E };
                        j /= this.arcScaleX_ * a;
                        E /= this.arcScaleY_ * a;
                        var ag = u.max(j, E);
                        ab = 2 * N.r0_ / ag;
                    ...
                    var Z = N.colors_;
                    Z.sort(function(H, i) { return H.offset - i.offset });
                    var T = Z.length;
                    var Y = Z[0].color;
                    var X = Z[T - 1].color;
        ...
        k.getCoords_ = function(E, j) {
            var i = this.m_;
            return{ x: a * (E * i[0][0] + j * i[1][0] + i[2][0]) - n, y: a * (E * i[0][1] + j * i[1][1] + i[2][1]) - n }
        };
        k.save = function() {
            var i = {};
            w(this, i);
        ...
        k.clip = function() {};
        k.arcTo = function() {};
        k.createPattern = function() { return new f };
